...
if (!validVersionTypes.includes(versionType) && !versionType.match(/^\d+\.\d+\.\d+$/)) {
  console.error(`Error: Version must be one of ${validVersionTypes.join(', ')} or a valid semver version (e.g. 1.2.3)`);
  process.exit(1);
}
...
function runCommand(command, options = {}) {
  try {
    ...
    return execSync(command, { encoding: 'utf8', stdio: options.stdio || 'pipe', cwd: ROOT_DIR, env: { ...process.env } });
  } catch (error) {
...
function versionExists(version) {
  try {
    const response = runCommand(`npm view isbn-bisac-tools@${version} version`, { ignoreError: true });
  ...
  if (versionExists(version)) {
    ...
    rl.question(`\nDo you want to automatically bump to the next patch version? (y/n) `, (answer) => {
      if (answer.toLowerCase() === 'y') {
        ...
        const newVersion = runCommand(`npm version patch --no-git-tag-version`).trim().replace('v', '');
